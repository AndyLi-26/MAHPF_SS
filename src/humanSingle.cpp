...
void HumanSingle::updatePath(const LLNode* goal, vector<PathEntry> &path)
{
...
Path HumanSingle::findOptimalPath(const PathTable& path_table, HumanSingle::Cost obj)
{
    Path path;
    num_expanded = 0;
    num_generated = 0;
    ...
    auto start = new HumanNode(start_location, 0,
            max(lowerbound, my_heuristic[start_location]), nullptr, 0, 0, false);

    num_generated++;
    if (obj==Cost::DIS)
    {
        start->dis_open_handle = dis_open_list.push(start);
        start->dis_focal_handle = dis_focal_list.push(start);
    }
    else
        start->conf_open_handle = conf_open_list.push(start);
    ...
    while (obj==Cost::DIS ? (!dis_open_list.empty()) : (!conf_open_list.empty()))
    {
        ...
        for (int next_location : next_locations)
        {
            ...
            auto next = new HumanNode(next_location, next_g_val, next_h_val,
                 ...
                 existing_next->num_of_conflicts > next->num_of_conflicts)) // or it remains the same but there's fewer conflicts
            {
                ...
                else
                {
                    if (obj==Cost::DIS)
                    {
                        ...
                        if (update_open)
                            dis_open_list.increase(existing_next->dis_open_handle);  // increase because f-val improved
                        if (add_to_focal)
                            existing_next->dis_focal_handle = dis_focal_list.push(existing_next);
                        if (update_in_focal)
                            dis_focal_list.update(existing_next->dis_focal_handle);  // should we do update? yes, because number of conflicts may go up or down
                    }
                    else
                    {
        ...
        const vector<Path*>& paths, int agent, int lowerbound, double w)
{
    ...
    // generate start and add it to the OPEN & FOCAL list
    auto start = new HumanNode(start_location, 0, max(lowerbound, my_heuristic[start_location]), nullptr, 0, 0, false);
    ...
    while (!dis_open_list.empty())
    {
        updateFocalList(); // update FOCAL if min f-val increased
        auto* curr = popNode(Cost::DIS);
        assert(curr->location >= 0);
        // check if the popped node is a goal
        if (curr->location == goal_location && // arrive at the goal location
                !curr->wait_at_goal && // not wait at the goal location
        ...
        for (int next_location : next_locations)
        {
                     ...
                     existing_next->num_of_conflicts > next->num_of_conflicts)) // or it remains the same but there's fewer conflicts
            {
                ...
                {
                    existing_next->copy(*next);
                    pushNode(existing_next,Cost::DIS);
                }
                else
                {
                    bool add_to_focal = false;  // check if it was above the focal bound before and now below (thus need to be inserted)
...
int HumanSingle::getTravelTime(int start, int end, const ConstraintTable& constraint_table, int upper_bound)
{
	...
	auto root = new HumanNode(start, 0, compute_heuristic(start, end), nullptr, 0);
	root->dis_open_handle = dis_open_list.push(root);  // add root to heap
	allNodes_table.insert(root);       // add root to hash_table (nodes)
	HumanNode* curr = nullptr;
	while (!dis_open_list.empty())
	{
		curr = dis_open_list.top(); dis_open_list.pop();
		...
		for (int next_location : next_locations)
		{
			...
			{  // if that grid is not blocked
				...
				if (next_g_val + next_h_val >= upper_bound) // the cost of the path is larger than the upper bound
					continue;
				auto next = new HumanNode(next_location, next_g_val, next_h_val, nullptr, next_timestep);
				auto it = allNodes_table.find(next);
				if (it == allNodes_table.end())
				{  // add the newly generated node to heap and hash table
					next->dis_open_handle = dis_open_list.push(next);
					allNodes_table.insert(next);
				}
					...
					if (existing_next->g_val > next_g_val)
					{
						...
						dis_open_list.increase(existing_next->dis_open_handle);
					}
				}
			}
		}
...
inline HumanNode* HumanSingle::popNode(HumanSingle::Cost obj)
{
    HumanNode* node;

    if (obj==Cost::DIS)
    {
        node = dis_focal_list.top(); dis_focal_list.pop();
        dis_open_list.erase(node->dis_open_handle);
    }
    else
        node = conf_open_list.top(); conf_open_list.pop();
...
inline void HumanSingle::pushNode(HumanNode* node, HumanSingle::Cost obj)
{
    if(obj==Cost::CONF)
        node->conf_open_handle = conf_open_list.push(node);
    else
        node->dis_open_handle = dis_open_list.push(node);
    ...
    if (obj==Cost::DIS)
        if (node->getFVal() <= w * min_f_val)
            node->dis_focal_handle = dis_focal_list.push(node);
}

void HumanSingle::updateFocalList()
{
	auto open_head = dis_open_list.top();
	if (open_head->getFVal() > min_f_val)
	{
		int new_min_f_val = (int)open_head->getFVal();
		for (auto n : dis_open_list)
		{
			if (n->getFVal() >  w * min_f_val && n->getFVal() <= w * new_min_f_val)
				n->dis_focal_handle = dis_focal_list.push(n);
		}
