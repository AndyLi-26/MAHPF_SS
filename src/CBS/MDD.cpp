...
Paths MDD::getAllPaths(int c) const
{
    Paths allPs;
    Path curP;
    assert (levels[0].size()==1);
    assert (levels[0].front()->location==solver->start_location);
    getAllPathsAux(levels[0].front(),curP,allPs, c);
    return allPs;
...
void MDD::getAllPathsAux(const MDDNode* n, Path curP, list<Path>& allP, int c) const
{
    ...
    for (const MDDNode* child: n->children)
    {
        curP.emplace_back(child->location);
        getAllPathsAux(child,curP,allP, c);
        curP.pop_back();
    }
