...
#include "lacam/post_processing.hpp"
...
class LNS
{
    ...
    private:
        ...
        lacamInstance preparelacamProblem(Instance instance);
        void updatelacamResult(Solution sol);

        bool generateNeighborByRandomWalk();
        //bool generateNeighborByStart();
        bool generateNeighborByIntersection(bool temporal = true);

        AgentID findMostDelayedAgent();
        AgentID findRandomAgent() const;
        void randomWalk(AgentID id, int start_location, int start_timestep,
                set<AgentID>& neighbor, int neighbor_size, int upperbound);
};
